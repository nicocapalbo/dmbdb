...
function downloadOnboardingLog() {
  const blob = new Blob(
    [parsedLogs.value.map(log => `${log.timestamp.toLocaleString()} [${log.level}] ${log.process}: ${log.message}`).join('\n')],
    { type: 'text/plain' }
  )
    ...
    <div class="flex-1 min-h-0 overflow-hidden">
      ...
      >
        <div v-for="log in filteredLogs" :key="log.id">
          <span class="text-gray-500">{{ log.timestamp.toLocaleString() }}</span>
